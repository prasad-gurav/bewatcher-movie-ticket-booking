			...
			<div>
				...
				<div className="flex gap-3 px-6 py-2">
					<button
						...
						className={`border-2  hover:bg-blue-500 border-blue-500 px-2 rounded-full ${showDate === todayDate ? "bg-blue-500" : ""} `}
					>
					...
					<button
						...
						className={`border-2  hover:bg-blue-500 border-blue-500 px-2 rounded-full ${showDate === tommorowsDate ? "bg-blue-500" : ""} `}
					>
				...
				{showtimes.map((show) => {
					return (
						<div
							...
							className={`${Number(currentHour) >= Number(show.split(":")[0]) && showDate === todayDate ? "text-gray-500" : "cursor-pointer text-blue-500 "} border-2   border-blue-500 px-2`}
						>
							{show}
						</div>
					);
