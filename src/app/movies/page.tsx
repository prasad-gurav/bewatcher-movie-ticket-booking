            ...
            <div className='flex flex-wrap gap-4   items-center px-10'>
                {moviesData.map((movie, index) => (
                    <div key={index} className='w-[200px]'>
                        ...
                        <button onClick={(e: MouseEvent<HTMLButtonElement>) => handleSetMovie(e, Number(moviesData.indexOf(movie)))} className='cursor-pointer bg-blue-500 px-4 py-2 rounded-md my-2'>BookMyTicket</button>
                    </div>
                ))}
            </div>
        </div>
